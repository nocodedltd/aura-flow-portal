...
const HowItWorks = () => {
  ...
  const details = {
    1: ["Initial discovery meeting", "Business needs analysis", "Current workflow assessment", "Opportunity identification"],
    2: ["Solution architecture blueprint", "Technology selection", "Implementation roadmap", "ROI projection"],
    3: ["Agile development", "Iterative prototyping", "Quality assurance", "Continual feedback loops"],
    4: ["Ongoing performance analysis", "Optimisation recommendations", "Regular review meetings", "Adaptation to evolving business needs"]
  };
  return <main>
        ...
        <div className="container">
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
            <div className="lg:col-span-2">
              <div ref={sidebarRef} className="bg-card rounded-lg border border-border p-6 sticky top-24">
                <h2 className="text-2xl font-semibold mb-6">The Bespoke Process</h2>
                
                <div className="space-y-4">
                  {steps.map((step, index) => <button key={step.id} ref={el => stepRefs.current[index] = el} id={`sidebar-step-${step.id}`} onClick={() => handleStepClick(step.id)} className={`w-full text-left p-4 rounded-md transition-all duration-300 ${activeStep === step.id ? "bg-primary text-primary-foreground" : "bg-muted/50 hover:bg-muted"}`}>
                      <div className="flex items-center">
                        <div className={`h-8 w-8 rounded-full border-2 flex items-center justify-center mr-3 ${activeStep === step.id ? "border-primary-foreground" : "border-muted-foreground"}`}>
                          {step.id}
                        </div>
            ...
            <div className="lg:col-span-3" ref={contentRef}>
              <div className="space-y-8">
                {steps.map((step, index) => <div key={step.id} id={`step-${step.id}`} className={cn("scroll-mt-24 transition-all duration-300", activeStep === step.id ? "opacity-100" : "opacity-60")}>
                    <ProcessStep number={step.id} title={step.title} description={step.description} isActive={activeStep === step.id} isLast={index === steps.length - 1} />
                  ...
                  duration: 0.3
                        ...
                        <Card className="p-4 bg-muted/50">
                          ...
                          <ul className="list-disc list-inside space-y-1 text-muted-foreground">
                            {details[step.id as keyof typeof details].map((item, i) => <li key={i}>{item}</li>)}
                          </ul>
                        </Card>
                      </motion.div>}
                  </div>)}
